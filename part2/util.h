...
typedef struct arguments_tcb ARGST;
 ...
 *  io_buffer
 *
 *	A lock-free buffer implementation for peripherals. 
 *
 *******************************************************/

#define IO_BUFFER_CAPACITY (16*1024)

/*
	Algorithm: This is a ring buffer of bytes, with transfers following three stages:

	(a) reserve:   change buffer state to 'reserve' a number of locations (data or free space)
	(b) transfer:  transfer from/to the reserved locations
	(c) release:   release the reserved locations

	The buffer state is described by two io_buffer_segment objects. One describes the 
	available data and the other the empty space.

	While this is slightly redundant (one segment can be computed from the other), 
	it helps with concurrency: because io_buffer_segment is 32 bits long, it can be 
	updated atomically, by a compare-exchange operation.
	
	Also, once the 'reserve' stage is completed, the corresponding locations in storage
	are not going to be accessed by another call, until the release stage is executed,
	or the reservation is reversed, with "unreserve"

 */

typedef struct {
	uint16_t position;					/* position of head of read/tail of write segment */
	uint16_t available;					/* amount of data available at this segment */
} io_buffer_segment;


typedef struct {
	io_buffer_segment data, space;		/* state objects */
	char storage[IO_BUFFER_CAPACITY];	/* storage area */
} io_buffer;


/*
	This operation reserves bytes from the given segment, for transfer.
	The operation is atomic and lock-free.

	The function returns the minimum of 'size' and 'available'.
	Also, it stores the current position in variable *pos.

	Finally, if n is the value returned, it updates the io_buffer_segment
	object as follows:
	
	position += n  (modulo IO_BUFFER_CAPACITY)
	available -= n
 */
...
/*
	This operation unreserves bytes from the given segment.
	The operation is atomic and lock-free.

	When the operation follows the last 'reserve' operation (and possible get/put
	operations), the state of the buffer is returned to the one before the reserve.
	However, once a second reserve on this buffer has followed, unreserve is not
	safe any more.

	Note that the other segment's operations do not affect the ability to unreserve, 
	as they do not affect the reservation.
...
/*
	Initialize io_buffer object
 */
void io_buffer_init(io_buffer* this);


/*
	Transfer size bytes from this buffer, starting at location pos. 
	State is not updated. Paremeters pos and size must have been
	returned by a previous call to io_buffer_segment_reserve() on
	this->data.

	A complete read(buf, bufsize) operation can be done as follows:

	uint pos;
	uint size = io_buffer_segment_reserve(& this->data, bufsize, &pos); 

	io_buffer_get(this, pos, buf, size);
...
/*
	Write size bytes into this buffer, starting at location pos.
	State is not updated. Paremeters pos and size must have been
	returned by a previous call to io_buffer_segment_reserve() on
	this->space.

	A complete write(buf, bufsize) operation can be done as follows:

	uint pos;
	uint size = io_buffer_segment_reserve(& this->space, bufsize, &pos); 

	io_buffer_put(this, pos, buf, size);
...
/*
	Read up to bufsize bytes from this buffer to buf.
	The operation is lock-free.
...
/*
	Write up to bufsize bytes from buf into this buffer. 
	The operation is lock-free.
